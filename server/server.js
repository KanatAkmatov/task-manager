...
server.get('/api/v1/tasks/:category', async (req, res) => {
  ...
  const tasks = filterTaskKey(await read(category))
  res.json(tasks)
})
...
server.get('/api/v1/tasks/:category/:timespan', async (req, res) => {
  ...
  // eslint-disable-next-line no-underscore-dangle
  const timeSpanTask = tasks.filter((el) => el._createdAt + periodOfTime[timespan] > +new Date())
  res.json(timeSpanTask)
})
